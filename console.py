...
class HBNBCommand(cmd.Cmd):
    ...
    It represents the command interpretor and the control center of this project.It defines function for all commands inputted in the console

    It sub-classes Python's 'cmd.Cmd' class which provides a simple framework for writing line orirnted command interpreters.
    """
    ...
    def handle_empty_line(self, line):
        """
        Eliminates all the empty lines
        """
        return False
        ...
        if attr_k in class_attr.keys():
            try:
                attr_v = type(class_attr[attr_k])(attr_v)
                    ...
                    print("You have entered the wrong value type, try Again!!!")
            return
    ...
    def my_count(self, class_n):
        """
        This method counts instances of a certain class.
        """
        count_instance = 0
    ...
    def default(self, line):
        """This method takes care of the following commands:
        <class name>.all()
        <class name>.count()
        <class name>.show(<id>)
        ...
        Description:
            Create a list representation of functional models
